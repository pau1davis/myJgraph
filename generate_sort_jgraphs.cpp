...
vector<int> parseValues(int argc, char *argv[]) {
  if (argc < 4) {
    throw runtime_error(
        "Provide values after sort type. Example: bubble 5 1 4 2 8 or bubble 5,1,4,2,8");
  }
...
vector<SortStep> quickSortSteps(const vector<int> &input) {
  ...
  function<void(int, int)> quickSortRange = [&](int lo, int hi) {
    ...
    for (int j = lo; j < hi; ++j) {
      if (arr[j] <= pivotValue) {
        ...
        if (i != j) {
          ...
          steps.push_back({stepIndex++,
                           arr,
                           "partition [" + to_string(lo) + ":" + to_string(hi) +
                               "] pivot " + to_string(pivotValue) +
                               " swap " + to_string(i) + "<->" + to_string(j),
                           {j, hi},
                           {i, j},
                           {i, j}});
        } else {
    ...
    if (pivotTarget != hi) {
      ...
      steps.push_back({stepIndex++,
                       arr,
                       "pivot swap " + to_string(pivotTarget) + "<->" +
                           to_string(hi),
                       {pivotTarget, hi},
                       {pivotTarget, hi},
                       {pivotTarget, hi}});
...
string toJgraph(const SortStep &step, const string &sortType) {
  ...
  out << "y_translate " << fixed << setprecision(3) << yTranslate
    << "\n";
  out << "xaxis min 0 max " << xMax << " size " << graphWidth
    << " nodraw\n";
  out << "yaxis min 0 max 3 size " << graphHeight << " nodraw\n";
  out << "newstring hjc x " << fixed << setprecision(3)
      << (xMax / 2.0) << " y 2.8 fontsize 12 font Helvetica-Bold : "
      << sortType << " step " << step.step << " - " << step.note << "\n";
...
filesystem::path createRunDir(const string &sortType,
  ...
  auto stamp = chrono::duration_cast<chrono::seconds>(
...
int writeSteps(const filesystem::path &runDir, const string &sortType,
  ...
  for (const auto &step : steps) {
    ...
    fileName << "step_" << setw(4) << setfill('0') << step.step
             << ".jgr";
...
void convertJgraphs(const filesystem::path &runDir) {
  ...
  if (!commandExists(jgraphCmd)) {
    throw runtime_error("jgraph executable not found (expected ./jgraph or jgraph in PATH).");
  }
  ...
  for (const auto &jgrFile : jgrFiles) {
    ...
    if (system(renderCmd.c_str()) != 0) {
      throw runtime_error("Failed to render PostScript for " + jgrFile.string());
    }
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    if (sortType == "bubble") {
      ...
      throw runtime_error("Sort type must be 'bubble', 'merge', or 'quick'.");
    }
