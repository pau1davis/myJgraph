...
demo: $(TARGET)
	...
	magick -density 180 "$(DEMO_DIR)/bubble_swap.pdf[0]" -quality 92 $(DEMO_DIR)/bubble_swap.jpg
	cp $(DEMO_DIR)/bubble_swap.jpg $(IMG_DIR)/bubble_swap.jpg
	magick -density 180 "$(DEMO_DIR)/bubble_late_pass.pdf[0]" -quality 92 $(DEMO_DIR)/bubble_late_pass.jpg
	cp $(DEMO_DIR)/bubble_late_pass.jpg $(IMG_DIR)/bubble_late_pass.jpg
	magick -density 180 "$(DEMO_DIR)/merge_mid_merge.pdf[0]" -quality 92 $(DEMO_DIR)/merge_mid_merge.jpg
	cp $(DEMO_DIR)/merge_mid_merge.jpg $(IMG_DIR)/merge_mid_merge.jpg
	magick -density 180 "$(DEMO_DIR)/merge_near_final.pdf[0]" -quality 92 $(DEMO_DIR)/merge_near_final.jpg
	cp $(DEMO_DIR)/merge_near_final.jpg $(IMG_DIR)/merge_near_final.jpg
	magick -density 180 "$(DEMO_DIR)/quick_partition.pdf[0]" -quality 92 $(DEMO_DIR)/quick_partition.jpg
	cp $(DEMO_DIR)/quick_partition.jpg $(IMG_DIR)/quick_partition.jpg
	magick -density 180 "$(DEMO_DIR)/quick_pivot_settle.pdf[0]" -quality 92 $(DEMO_DIR)/quick_pivot_settle.jpg
